...
G_{n} = \{x\mid (x,y)\in G \text{ for some }y\in V_{n}\}$$If $x\in G_{n}$, then let $(x,y)\in G\cap (X\times V_{n})$, as $G$ is open, there is $(x,y)\in U\times V\subseteq G\cap (X\times V_{n})$ and hence $x\in U\subseteq G_{n}$, which means $G_{n}$ is open.
